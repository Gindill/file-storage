...
abstract class Model
{
    ...
    public function __construct($id = NULL)
    {
        ...
        else
        {
            ...
            {
                // TODO инициализация в соответствии с $field_types;
            ...
            if (in_array($name, array_keys(static::$behaviours)))
            {
                ...
                if (static::$behaviours[$name]['type'] === 'one')
                {
                    if (isset(static::$behaviours[$name]['key']))
                    {
                        ...
                        $class_name = static::$behaviours[$name]['class'];

                        $object = new $class_name($this->$id);
    ...
    public static function order($order, $where = '1', $trend = 'ASC')
    {
      if (!isset($where))
      {
        $where = '1';
      ...
      if (is_array($where))
      {
        ...
        $query = "SELECT " . static::get_query_fields_list(true) .
                " FROM `" . static::$table . "` WHERE {$where}" .
                " ORDER BY `" . static::$table . "`.`{$order}` {$trend}";
